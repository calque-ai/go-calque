...
func TestToolsConvenienceIntegration(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:            "complete tools flow - search",
			input:           "search for golang tutorials",
			llmResponse:     `{"selected_tool": "search", "confidence": 0.9}`,
			expectedTool:    "search",
			expectExecution: true,
			description:     "Tools() convenience should handle full detect+execute flow",
		},
		{
			name:            "complete tools flow - no tool",
			input:           "hello world",
			llmResponse:     `{"selected_tool": null, "confidence": 0.1}`,
			expectedTool:    "",
			expectExecution: false,
			description:     "Tools() should pass through when no tool needed",
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Note: We can't fully test Tools() convenience function here because
			// it requires ToolRegistry which needs a real MCP client connection.
			// But we can verify the individual components work together:

			// Simulate the Tools() chain: ToolRegistry → DetectTool → ExtractParams → ExecuteTool
			// We'll test DetectTool + ExecuteTool since we have context
			detectHandler := DetectTool(mockLLM)
			executeHandler := ExecuteTool()
