	...
	tests := []struct {
	...
	}{
		{
			...
			property: &jsonschema.Schema{
				...
				MinLength:   helpers.Uint64Ptr(1),
				MaxLength:   helpers.Uint64Ptr(100),
				Pattern:     "^[a-zA-Z0-9\\s]+$",
			},
			expectedType:   "string",
			expectedDesc:   "Search query",
			expectedMinLen: helpers.IntPtr(1),
			expectedMaxLen: helpers.IntPtr(100),
		},
		{
			...
			expectedMin:     helpers.Float64Ptr(1.0),
			expectedMax:     helpers.Float64Ptr(100.0),
			expectedDefault: 10,
		},
		{
